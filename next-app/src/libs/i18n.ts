...
function getOptions (lang = lngdef, ns: any = nsdef) {
  return {
...
const initI18next = async (ns: string[] | string, lang: string) => {
  if (!inst) {
    inst = createInstance()
    await inst
      .use(initReactI18next)
      .use(resourcesToBackend((lang: string, _: string) => {
        log.trace('RESOURCE:', lang, ns)
        const ret = new Promise<any>(async (resolve) => {
          ...
          if (ns instanceof Array) {
            for (let itm of ns) {
              try {
                log.trace('GET-RESOURCE:', lang, itm)
                messages = values.putAll(messages, (await import(`@/locales/${lang}/${itm}`)).default)
              } catch (e) {
            ...
            try {
              log.trace('GET-RESOURCE:', lang, ns)
              messages = (await import(`@/locales/${lang}/${ns}`)).default
            } catch (e) {
              log.trace('E:', asAny(e).message)
            }
          }
          log.trace('MESSAGES:', lang, messages)
          return resolve(messages)
        })
        return ret
      }))
      /** 메시지를 한번에 가져오기 위해 최초 네임스페이스만 인식한다. */
      .init(getOptions(lang, ns instanceof Array ? ns[0] : ns))
  }
  return inst
}

function getPersistedLang() {
  let lang = C.UNDEFINED
  let o: any
...
var setPersistedLang = lodash.debounce((lang: string) => {
  let o: any
  if (typeof localStorage !== 'undefined') {
...
async function getTranslation(ns: string[] | string, lang?: string, opt: any = {}) {
  if (!lang) {
    lang = getPersistedLang()
  } else {
    setPersistedLang(lang)
  ...
  if (!lang) { lang = lngdef }
  if (!ns) { ns = nsdef }
  log.trace('GET-TRANSLATION:', ns, lang, opt)
  nscur = ns
  lngcur = lang
...
const initI18N = async (ns: string[] | string, lang?: string) => { await getTranslation(ns, lang) }
const changeLang = async (lang: string) => { await getTranslation(nscur, lang) }
$t.init = initI18N
$t.lang = changeLang
$t.persist = persist
